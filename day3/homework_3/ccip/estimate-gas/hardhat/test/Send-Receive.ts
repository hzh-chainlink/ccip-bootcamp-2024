...
describe("Sender and Receiver", function () {
  ...
  async function deployFixture() {
    ...
    const link = await BurnMintERC677.deploy("ChainLink Token", "LINK", 18, BigInt(1e27));
  ...
  it("should CCIP message from sender to receiver", async function () {
    ...
    for (const iterations of testParams) {
      await sender.sendMessagePayLINK(chainSelector, receiver, iterations, gasLimit);
    ...
    gasUsageReport.forEach((report) => {
      console.log("Number of iterations %d - Gas used: %d", report.iterations, report.gasUsed);
    });

    const maxGasUsed = 24077;
    const gasLimitWithBuffer = Math.ceil(maxGasUsed * 1.1); // add 10% buffer
    console.log("TransferUSDC transferUsdc _gasLimit should be", gasLimitWithBuffer);
  });
});
