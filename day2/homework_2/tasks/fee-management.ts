...
task(`fund`, `Transfers the provided amount to the CrossChainNameServiceRegister smart contracts for paying CCIP fees`)
  ...
  .setAction(async (taskArguments: TaskArguments, hre: HardhatRuntimeEnvironment) => {
    ...
    console.log(
      `ℹ️  Attempting to send ${amount} wei to CrossChainNameServiceRegister smart contract (${ccnsRegisterAddress}) on the ${hre.network.name} blockchain`
    );
    spinner.start();
...
task(`get-balance`, `Gets balance of CrossChainNameServiceRegister smart contracts in wei`)
  ...
  .setAction(async (taskArguments: TaskArguments, hre: HardhatRuntimeEnvironment) => {
    ...
    if (!ccnsRegisterAddress) {
      console.error(
        `❌ CrossChainNameServiceRegister address is undefined. Try to provide the address of a CrossChainNameServiceRegister smart contract via --register flag.`
      );
      return 1;
    }
...
task(`withdraw`, `Withdraws all coins from the CrossChainNameServiceRegister smart contracts`)
  ...
  .setAction(async (taskArguments: TaskArguments, hre: HardhatRuntimeEnvironment) => {
    ...
    console.log(
      `ℹ️  Attempting to call the withdraw function on the CrossChainNameServiceRegister smart contract on the ${hre.network.name} blockchain`
    );
    spinner.start();
