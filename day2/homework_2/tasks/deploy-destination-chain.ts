...
task(
  `deploy-destination-chain-step1`,
  `Sets up the Cross Chain Name Service on the destination network by deployinh Lookup and Receiver smart contracts and linking them`
)
  ...
  .setAction(async (taskArguments: TaskArguments, hre: HardhatRuntimeEnvironment) => {
    if (hre.network.name === hre.config.defaultNetwork) {
      console.error(
        `❌ CrossChainNameServiceReceiver can not be deployed on the source chain. Source chain - ${hre.config.defaultNetwork}`
      );
      return 1;
    }

    const ccnsRegisterAddress = taskArguments.register
      ? taskArguments.register
      : getDeploymentInfo(hre.config.defaultNetwork).ccnsRegister;

    if (!ccnsRegisterAddress) {
      console.error(
        `❌ CrossChainNameServiceRegister address is undefined. Did you run the "npx hardhat deploy-source-chain" command? Was the "${join(
          __deploymentsPath,
          `${hre.config.defaultNetwork}.json`
        )}" file generated? Try to provide the address of a CrossChainNameServiceRegister smart contract via --register flag.`
      );
      return 1;
    }
    ...
    console.log(
      `ℹ️  Attempting to deploy CrossChainNameServiceLookup on the ${hre.network.name} blockchain using ${deployer.address} address`
    );
    spinner.start();
    ...
    spinner.stop();
    console.log(
      `✅ CrossChainNameServiceLookup deployed at address ${ccnsLookup.address} on ${hre.network.name} blockchain`
    );
    ...
    const sourceChainSelector = taskArguments.sourceChainSelector
      ...
      : getRouterConfig(hre.config.defaultNetwork).chainSelector;
    ...
    try {
      ...
      console.log(
        `ℹ️  Saving the CrossChainNameReceiver address to ${filePath} file failed, please save it manually from previous log, you will need it for further tasks`
      );
...
task(
  `deploy-destination-chain-step2`,
  `Enables previously depolyed CrossChainNameServiceReceiver contract on the source chain`
)
  ...
  .setAction(async (taskArguments: TaskArguments, hre: HardhatRuntimeEnvironment) => {
    ...
    const destinationChainSelector = taskArguments.destinationChainSelector
      ? taskArguments.destinationChainSelector
      : getRouterConfig(taskArguments.receiverNetwork).chainSelector;
    const ccnsReceiverAddress = taskArguments.receiver
      ? taskArguments.receiver
      : getDeploymentInfo(taskArguments.receiverNetwork).ccnsReceiver;

    const ccnsRegister: CrossChainNameServiceRegister = CrossChainNameServiceRegister__factory.connect(
      ccnsRegisterAddress,
      deployer
    );
    ...
    console.log(
      `ℹ️  Attempting to call the enableChain function on the CrossChainNameServiceRegister smart contract on the ${hre.network.name} blockchain`
    );
    spinner.start();
